    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-4 px-4 py-2">
        {Array.from({ length: columns }).map((_, i) => (
          <Skeleton key={i} className="h-3 flex-1 dark:bg-gray-700" />
        ))}
      </div>
      {/* rows */}
      {Array.from({ length: rows }).map((_, row) => (
        <div key={row} className="flex gap-4 px-4 py-3 border-t border-gray-100 dark:border-gray-800">
          {Array.from({ length: columns }).map((_, col) => (
            <Skeleton key={col} className="h-4 flex-1 dark:bg-gray-700" style={{ opacity: 1 - col * 0.1 }} />
          ))}
        </div>
      ))}
    </div>
...
export function SkeletonCard() {
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 p-5 space-y-3">
      <Skeleton className="h-4 w-1/3 dark:bg-gray-600" />
